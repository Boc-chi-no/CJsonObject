...
namespace neb
{
    ...
    std::string CJsonObject::operator()(const std::string& strKey, std::string precision, int divisor) const
    {
        cJSON* pJsonStruct = NULL;
        if (m_pJsonData != NULL)
        {
        ...
        else if (pJsonStruct->type == cJSON_Int)
        {
            ...
            if (pJsonStruct->sign == -1)
            {
                ...
                {
                    if (divisor > 1)
                        snprintf(szNumber, sizeof(szNumber), "%d", (int32)pJsonStruct->valueint/divisor);
                    else
                ...
                else
                {
                    ...
                    if (divisor > 1)
                        snprintf(szNumber, sizeof(szNumber), "%ld", (int64)pJsonStruct->valueint / divisor);
                    else
                        snprintf(szNumber, sizeof(szNumber), "%ld", (int64)pJsonStruct->valueint);
#else
                    if (divisor > 1)
                        snprintf(szNumber, sizeof(szNumber), "%lld", (int64)pJsonStruct->valueint / divisor);
                    else
            ...
            else
            {
                ...
                {
                    if (divisor > 1)
                        snprintf(szNumber, sizeof(szNumber), "%u", (uint32)pJsonStruct->valueint / divisor);
                    else
...
#if LLONG_MAX==LLONG_MAX
                    if (divisor > 1)
                        snprintf(szNumber, sizeof(szNumber), "%lu", pJsonStruct->valueint/divisor);
                    else
                        snprintf(szNumber, sizeof(szNumber), "%lu", pJsonStruct->valueint);
#else
                    if (divisor > 1)
                        snprintf(szNumber, sizeof(szNumber), "%llu", pJsonStruct->valueint/divisor);
                    else
    ...
    std::string CJsonObject::operator()(unsigned int uiWhich, std::string precision, int divisor) const
    {
        cJSON* pJsonStruct = NULL;
        if (m_pJsonData != NULL)
        {
